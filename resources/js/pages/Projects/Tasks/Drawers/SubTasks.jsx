...
import { IconPlus, IconIndentIncrease, IconIndentDecrease, IconTrash } from '@tabler/icons-react';
...
export default function SubTasks({ task }) {
  ...
  const persistReorder = async tree => {
    ...
    await axios.post(route('projects.tasks.subtasks.reorder', [project.id, task.id]), {
      items: list,
    });
  };
  ...
  const addNode = async (parentId = null) => {
    ...
    const { data } = await axios.post(
      route('projects.tasks.subtasks.store', [project.id, task.id]),
      payload
    );
    setNodes(prev => {
      ...
      >
        ...
        >
          <Checkbox
            ...
            onChange={e =>
              updateNode(node.id, {
                completed_at: e.currentTarget.checked ? new Date().toISOString() : null,
              })
            }
          />
          {editing ? (
        ...
        >
          ...
          <DateInput
            ...
            onChange={d =>
              updateNode(node.id, { due_on: d ? dayjs(d).format('YYYY-MM-DD') : null })
            }
          />
          <NumberInput
            size='xs'
    ...
    >
      ...
      {loading ? (
        ...
        <Text
          ...
          c='dimmed'
        >
          No subtasks yet. Use Add subtask to create one.
        </Text>
      ) : (
