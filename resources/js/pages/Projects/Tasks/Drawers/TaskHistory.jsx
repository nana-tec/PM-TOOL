...
export default function TaskHistory({ task, onRestored }) {
  ...
  const savePersistedMode = (field, mode) => {
    ...
    try {
      localStorage.setItem('taskHistory.viewModes', JSON.stringify(next));
    } catch {}
  };
  ...
  const ensureDiffLibs = async () => {
    ...
    try {
      const [diffMod, htmlMod] = await Promise.all([import('diff'), import('htmldiff-js')]);
      const libs = {
  ...
  const restore = async auditId => {
    ...
    try {
      ...
      showNotification({
        ...
        message: 'Could not restore this version.',
      });
    } finally {
      setRestoring(null);
    }
  ...
  const getFieldViewMode = (auditId, field, defaultMode = 'inline') =>
    viewMode[`${auditId}:${field}`] || persistedMode[field] || defaultMode;
  ...
  const labelNamesByIds = arr => {
    const ids = Array.isArray(arr)
      ? arr.map(x => (x && typeof x === 'object' ? (x.id ?? x) : x))
      : [];
        ...
        {parts.map((p, idx) => {
          ...
          if (p.removed)
            return (
              <span
                key={idx}
                style={{ backgroundColor: 'rgba(239,68,68,0.2)', textDecoration: 'line-through' }}
              >
                {p.value}
              </span>
            );
  ...
  return (
    <>
      ...
      <Modal
        ...
        title={`Task #${task?.number} history`}
        size='lg'
      >
        {/* Legend */}
        <Group
        ...
        >
          ...
          <Text size='xs'>
            <span style={{ backgroundColor: 'rgba(34,197,94,0.2)', padding: '0 4px' }}>added</span>
          </Text>
          <Text size='xs'>
            <span
              style={{
          ...
          <Text
            ...
            c='dimmed'
          >
            Use the view toggle on long text to switch Inline/Side-by-side.
          </Text>
        </Group>
          ...
          >
            ...
            <Group gap='xs'>
              <Button
                ...
                onClick={async () => {
                  await ensureDiffLibs();
                  setExpanded(items.map(i => String(i.id)));
                }}
              >
                Expand all
              </Button>
          ...
          >
            ...
            >
              {items.map(h => {
                ...
                const restoreSelectable = changedKeysAll.filter(k =>
                  Object.keys(allowedFieldLabels).includes(k)
                );
                const restoreOptions = restoreSelectable.map(k => ({
                  value: k,
                  label: allowedFieldLabels[k],
                }));
                      ...
                      {isOpen && (
                        ...
                        >
                          {changedKeysAll.map(field => {
                              ...
                              const actions = (
                                ...
                                >
                                  <SegmentedControl
                                    size='xs'
                                    value={mode}
                                    onChange={m => setFieldViewMode(h.id, field, m)}
                                    data={[
                            ...
                            const isLongText =
                              isText && ((oldVal?.length || 0) > 80 || (newVal?.length || 0) > 80);
                            if (isText) {
                              const mode = getFieldViewMode(
                                h.id,
                                field,
                                isLongText ? 'inline' : 'inline'
                              );
                              const actions = (
                                <Group
                                  justify='space-between'
                                  ...
                                  <Group gap='xs'>
                                    <Tooltip label='Copy Old'>
                                      <ActionIcon
                                        variant='light'
                                        onClick={() =>
                                          copyToClipboard(`Old ${label}`, String(oldVal ?? ''))
                                        }
                                      >
                                        <IconCopy size={14} />
                                      </ActionIcon>
                                    ...
                                    <Tooltip label='Copy New'>
                                      <ActionIcon
                                        variant='light'
                                        onClick={() =>
                                          copyToClipboard(`New ${label}`, String(newVal ?? ''))
                                        }
                                      >
                                        <IconCopy size={14} />
                                      </ActionIcon>
                          ...
                          {h.event === 'updated' && changedKeysAll.length > 0 && (
                            ...
                            >
                              Changed fields:{' '}
                              {changedKeysAll.map(k => allowedFieldLabels[k] || k).join(', ')}
                            </Text>
                          )}
                        </Stack>
                      )}
