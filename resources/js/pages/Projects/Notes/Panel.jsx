...
export default function NotesPanel({ projectId }) {
  ...
  const fetchNotes = async (page = 1) => {
    setLoading(true);
    try {
      const { data } = await axios.get(route('projects.notes', projectId), {
  ...
  const addNote = async () => {
    ...
    setSaving(true);
    try {
      const { data } = await axios.post(route('projects.notes.store', projectId), {
        content: newContent,
      });
  ...
  const saveEdit = async noteId => {
    ...
    setSaving(true);
    try {
      const { data } = await axios.put(route('projects.notes.update', [projectId, noteId]), {
        content: editContent,
      });
  ...
  const restoreFromHistory = async auditId => {
    ...
    try {
      const { data } = await axios.post(
        route('projects.notes.history.restore', [projectId, historyNoteId, auditId])
      );
                ...
                >
                  {editingId === note.id ? (
                    ...
                    >
                      ...
                      >
                        <Group gap='xs'>
                          {note.user && <Text fw={600}>{note.user.name}</Text>}
                          <Text
                            size='xs'
                            c='dimmed'
                          >
                  ...
                  >
                    <Group gap='xs'>
                      <Badge
                        ...
                        variant='light'
                        color={
                          h.event === 'created' ? 'green' : h.event === 'updated' ? 'blue' : 'gray'
                        }
                      >
                        {h.event}
                      </Badge>
