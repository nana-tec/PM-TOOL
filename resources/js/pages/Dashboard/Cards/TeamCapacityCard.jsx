...
export default function TeamCapacityCard({ capacityData }) {
  ...
  return (
    <Card
      ...
      onClick={() => redirectTo('reports.member-report')}
    >
      <Group
      ...
      >
        <Stack gap={4}>
          <Text
            ...
            fw={600}
          >
            Avg Planned Utilization
          </Text>
          <Group gap='xs'>
            <Text
              size='lg'
              fw={700}
              c={getUtilColor(summary.avg_planned_util)}
            >
          ...
          >
            ...
            >
              ...
              <Text
                ...
                c='orange'
              >
                {summary.at_capacity + summary.over_capacity}
              </Text>
            </Group>
          </Stack>
        </Tooltip>
      ...
      {overCapacityMembers.length > 0 && (
        ...
        >
          ...
          {overCapacityMembers.slice(0, 3).map(member => (
            <Tooltip
              key={member.id}
              label={`${member.name}: ${member.planned_utilization}%`}
            >
              <Text
                size='sm'
                c='red'
          ...
          {overCapacityMembers.length > 3 && (
            <Text
              ...
              c='dimmed'
            >
              +{overCapacityMembers.length - 3} more
            </Text>
          )}
        </Group>
      )}
