...
class VcsGithubClient implements VcsClientInterface
{
    ...
    {
        $payload = ['json' => [
            'head' => $sourceBranch, // may be owner:branch for forks
            'base' => $targetBranch,
            'title' => $title,
            'body' => $body,
        ]];
        if (array_key_exists('draft', $options)) {
            $payload['json']['draft'] = (bool) $options['draft'];
        }
    ...
    public function mergeRequest(int|string $number, ?string $strategy = null): array
    {
        $json = new \stdClass;
        if ($strategy) {
            $method = strtolower($strategy);
            if (in_array($method, ['merge', 'squash', 'rebase'], true)) {
                $json = ['merge_method' => $method];
            }
        }
        $data = $this->api('PUT', '/pulls/'.urlencode((string) $number).'/merge', ['json' => $json]);
    ...
    public function getPullStatuses(int|string $number): array
    {
        ...
        if ($sha) {
            ...
            try {
                $checks = $this->api('GET', '/commits/'.urlencode($sha).'/check-runs');
                foreach ($checks['check_runs'] ?? [] as $cr) {
                    $conclusion = $cr['conclusion'] ?? null; // success, failure, neutral, cancelled, timed_out, action_required, stale, skipped
                    $status = $cr['status'] ?? null; // queued, in_progress, completed
                    $state = 'pending';
                    if ($status !== 'completed') {
                        $state = 'pending';
                    } else {
                        $state = match ($conclusion) {
                            'success' => 'success',
                            'failure', 'timed_out', 'cancelled', 'action_required' => 'failure',
                            'neutral', 'stale', 'skipped' => 'neutral',
                            default => 'pending',
                        };
                    }
                    $statuses[] = [
                        'context' => 'check:'.($cr['name'] ?? 'check'),
                        'state' => $state,
                        'description' => $cr['output']['title'] ?? $cr['output']['summary'] ?? null,
                        'url' => $cr['html_url'] ?? null,
                    ];
                }
            } catch (\Throwable $e) {
                // ignore checks API errors
            }
        }
    ...
    /**
     * GitHub branch protection required status contexts.
     */
    ...
    // Extra method not in interface but used conditionally by controller for GitHub
    public function markReadyForReview(int|string $number): array
    {
        $this->api('POST', '/pulls/'.urlencode((string) $number).'/ready_for_review');
